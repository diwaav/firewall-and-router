...
class iplb (object):
  ...
  def __init__ (self, connection, service_ip, servers = []):
    ...
    self.counter = 0 # a counter to figure out where we are in round robbin
  ...
  def _pick_server (self, key, inport):
    ...
    if (self.counter >= len(self.live_servers.keys())):
        self.counter = 0
    return (self.live_servers.keys()[self.counter])
